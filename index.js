...
const final2014 = fifaData.filter(e => (e.Year == 2014 && e.Stage == "Final"))[0];
...
function getYears(data, getFinals) {
    return getFinals(data).map(e => e.Year);
}
...
function getWinnersByYear(data, getYears, getWinners) {
    ...
    return years.map(function(yr, i) {
        return `In ${yr}, ${winners[i]} won the world cup!`;
    });
}
...
function getAverageGoals(finalsData) {
    const reducer = function(acc, e) {
        return acc + e["Home Team Goals"] + e["Away Team Goals"];
    }
    const total_goals = finalsData.reduce(reducer, 0);
    const avg = total_goals/finalsData.length;
    const rounded_avg = avg.toFixed(2);
    return rounded_avg;
}
